...
SLICE=50
OUTPUT_PATH = "output_test_clean_id.tsv"

KEY_PROMPT =    "Translate this english text to tatar\n" \
...
with open(OUTPUT_PATH, encoding="utf-8") as f:
    output_lines = [line.strip().split("\t")[2] for line in f if line.strip()][1:]
    ...
    if len(sys.argv) == 3:
        ...
        for i in range(0, len(new_output_lines), SLICE):
            ...
            if (i+SLICE <= len(new_output_lines)):
                resulting = [k for k in new_output_lines[i:i+SLICE]]
                # print(resulting)
                result = await detoxify_sentence(resulting)
                results.append(result)
            else:
                resulting = [k for k in new_output_lines[i:]]
                # print(resulting)
                result = await detoxify_sentence(resulting)
                results.append(result)
